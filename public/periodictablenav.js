	function terms_mouseover($origin,$adj,adj_posn) {
		console.log("Running terms_mouseover: " + adj_posn);	// debugging
		$origin_terms = $origin.find('.terms'); // get terms (orbitals, e.g. 3s2) in origin element
		switch (adj_posn) // Case statement: which is the element moused over
		{
		case "above":
			//adj_edge_to_hightlight = "bottom";
			//console.log("Element moused over: above");
			$adj.css({"border-bottom":"1px dashed DimGray"});
			$origin.css({"border-top":"1px dashed DimGray"})
			break;
		case "left":
			//adj_edge_to_hightlight = "right";
			//console.log("Element moused over: left");
			$adj.css({"border-right":"1px dashed DimGray"});
			$origin.css({"border-left":"1px dashed DimGray"})
			break;
		case "right":
			//adj_edge_to_hightlight = "left";
			//console.log("Element moused over: right");
			$adj.css({"border-left":"1px dashed DimGray"});
			$origin.css({"border-right":"1px dashed DimGray"})
			break;
		case "below":
			//adj_edge_to_hightlight = "top";
			//console.log("Element moused over: below");
			$adj.css({"border-top":"1px dashed DimGray"});
			$origin.css({"border-bottom":"1px dashed DimGray"})
			break;
		}
				// Would be better to store borders (top, left, right, bottom) in variable and only call $adj.css and $origin.css once,
...
//				$this.css({css_element:"5px solid green"});
		...
		//alert($(this).attr('id'));
		$adj_terms = $adj.find('.terms'); // get terms (orbitals, e.g. 4f10) in adjacent element
		$adj_terms.children().each(function() { // for each orbital term in adjacent element
			$adj_term = $(this);
			adj_term = $adj_term.text().replace(/\s/g, ''); // e.g., 4f10 [format: nlc where c = electron count]
			...
			if(adj_n != origin_n) {
				...
				$adj_term.find('span[class^="ec_"][class$="n"]').addClass('diff_n');
			}
			...
			if(adj_count != origin_count) {
				...
				$adj_term.find('span[class^="ec_"][class$="c"]').find('sup').addClass('diff_count');
			}
					
		}) // end $adj_terms.children().each
		$origin_terms.children().each(function() { // for each orbital term in origin element
			$origin_term = $(this);
			origin_term = $origin_term.text().replace(/\s/g, ''); // get the text of the origin term, e.g. 4f10
			...
			if (!adj_has_term) { // if adj element doesn't contain term with this l, highlight whole term in origin element
				$origin_term.addClass('new_orbital');
			}
		}) // end $origin_terms.children().each	
	}

	function terms_mouseout($origin,$adj) {
		...
		$adj.css({"border":"1px solid black"});
		$origin.css({"border":"1px solid black"});
	}
